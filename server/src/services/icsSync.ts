...
import { formatSyncError } from './syncErrors.js';

const activeIcsSyncs = new Set<string>();

export function scheduleIcsSync(connectionId: string): void {
  setImmediate(() => {
    void listIcsEventsForConnection(connectionId).catch((error) => {
      console.error(
        `ICS sync failed for ${connectionId}: ${formatSyncError(error)}`,
      );
    });
  });
}
...
export async function listIcsEventsForConnection(connectionId: string): Promise<void> {
  if (activeIcsSyncs.has(connectionId)) return;
  activeIcsSyncs.add(connectionId);
  ...
  try {
    ...
    console.log(`ICS sync started: ${row.feed_url}`);
    ...
    const items = await parseIcsEventsAsync(icsText, timeMin, timeMax);
    console.log(`ICS sync parsed ${items.length} events for ${row.feed_url}`);
    ...
    console.log(`ICS sync finished: ${row.feed_url}`);
  } finally {
    activeIcsSyncs.delete(connectionId);
  }
}
