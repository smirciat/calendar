...
REM Upload the Windows Android debug keystore to bering-dev for kiosk OTA signing.
REM Run from the Windows PC that built kiosk builds 15-17 (same signing key).
...
echo.
echo ========================================
echo  Family Calendar - Upload Kiosk Keystore
...
echo   app/scripts/setup-kiosk-signing.sh --import ~/.config/family-calendar/windows-debug.keystore
echo   app/scripts/build-kiosk.sh
echo   app/scripts/distribute-kiosk.sh
echo   pm2 restart family-calendar
echo.
echo Then on the wall tablet: 7-tap admin -^> Check for updates -^> Download and install
